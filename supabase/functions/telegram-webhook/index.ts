...
const AGENT_TOOLS: ToolDef[] = [
  ...
  },,
  {
    ...
    description: "Research playlist opportunities for a track on Spotify and SoundCloud. Use when asked to find playlists, research playlist opportunities, or pitch a track.",
  ...
  },
  {
    ...
    input_schema: { type: "object", properties: { playlist_id: { type: "string" }, status: { type: "string" }, notes: { type: "string" } }, required: ["playlist_id", "status"] }
  }
];
  ...
  try {
    ...
    if (text.toLowerCase() === "/triage") {
      ...
      try {
        ...
        await sendMessage(chatId, `✅ Done: \`${taskimport { serve } from "https://deno.land/std@0.168.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.49.1";
...
// ── FanFuel Hub cross-project caller ──
